...
namespace Themp
{
	...
	DirectX::XMFLOAT4 Normalize(const DirectX::XMFLOAT4& v);
	...
	float Clamp(float val, float a_min, float a_max);
	int Clamp(int val, int a_min, int a_max);
	unsigned int Clamp(unsigned int val, unsigned int a_min, unsigned  int a_max);
